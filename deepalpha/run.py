...
def build_parser():
    parser = argparse.ArgumentParser(
        description="X 实时情报系统 - 任意问题输入 + 市场情报抓取 + 独立关系图谱",
        usage='python3 run.py ["你的问题"] [选项]',
    )
...
def run_query(query: str, args):
    ...
    if not args.crawl:
        _analyze_existing_intel(query, decision, args)
        return
...
def _print_decision(decision: dict):
    ...
    for account in decision["top_accounts"]:
        print(f"    {account}")
    print("\n  top_event_phrases:")
    for phrase in decision["top_event_phrases"]:
        print(f"    {phrase}")
    print("\n  crawl_tasks:")
