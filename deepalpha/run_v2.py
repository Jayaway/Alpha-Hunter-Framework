...
from deepalpha.graph_engine import DEFAULT_GRAPH_FILE
...
def build_parser():
    ...
    parser.add_argument("--cookie-file", type=str, default="cookies/browser/x_cookie.json", help="Cookie文件路径")
...
def run_query(query: str, args):
    ...
    if args.dry_run:
        print("\n  [DRY RUN] 仅显示决策，不执行抓取。")
        from deepalpha.crawler_runner import print_tasks
        print("\n  将要执行的任务:")
        print("-" * 40)
    ...
    relevant_tweets = _filter_tweets_for_query(all_tweets, query, decision)
    cleaned_tweets = _clean_tweets(relevant_tweets, args, ai)
...
def _make_decision(query: str, args, ai):
    if args.ai:
        from deepalpha.ai_model import ai_decide
...
def _analyze_existing_intel(query: str, decision: dict, args):
    from deepalpha.graph_engine import generate_graph_data
    from deepalpha.intel_analyzer import analyze_history, print_history_report
...
def _clean_tweets(all_tweets: list, args, ai):
    ...
    if args.new_cleaner:
        from deepalpha.cleaner_v2 import clean_tweets as clean_tweets_v2
    ...
    if args.ai and ai is not None:
        from deepalpha.ai_model import ai_batch_clean
    ...
    from deepalpha.data_cleaner import Tweet as CleanTweet, clean_pipeline
...
def _judge_signals(cleaned_tweets: list, decision: dict, args):
    ...
    from deepalpha.signal_judge import judge_all_signals, print_signal_report
...
def _filter_tweets_for_query(tweets: list, query: str, decision: dict) -> list:
    terms = _relevance_terms(query, decision)
    if not terms:
        return tweets
    ...
    for tweet in tweets:
        haystack = " ".join(
            [
                str(tweet.get("content", "")),
                " ".join(str(item) for item in tweet.get("tags", []) or []),
                " ".join(str(item) for item in tweet.get("mentions", []) or []),
                str(tweet.get("handle", "")),
            ]
        ).lower()
        if any(term.lower() in haystack for term in terms):
            filtered.append(tweet)

    print(f"  相关性过滤: {len(tweets)} → {len(filtered)} 条")
    if not filtered:
        print("  相关性过滤后没有命中，后续判断将按数据不足处理。")
    return filtered


def _relevance_terms(query: str, decision: dict) -> list[str]:
    asset = decision.get("asset")
    terms = []
    if asset == "oil":
        terms.extend(["oil", "crude", "brent", "wti", "opec", "原油", "石油", "油价"])
    elif asset == "crypto":
        terms.extend(["crypto", "bitcoin", "btc", "ethereum", "eth", "加密", "比特币", "以太坊"])
        terms.extend(_extract_ticker_terms(query))
    elif asset == "gold":
        terms.extend(["gold", "xau", "黄金", "金价"])

    terms.extend(str(item) for item in decision.get("top_event_phrases", []) or [])
    return _dedupe_terms([term for term in terms if term])


def _extract_ticker_terms(query: str) -> list[str]:
    terms = []
