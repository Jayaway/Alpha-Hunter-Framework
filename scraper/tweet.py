    ...
    def __init__(
        ...
        if scrape_poster_details:
            try:
                ...
                for _ in range(3):
                    try:
                        actions.move_to_element(el_name).pause(0.5).perform()
                        cards = driver.find_elements("xpath", '//div[@data-testid="hoverCardParent"]')
                        if cards:
                            hover_card = cards[0]
                            break
                    except Exception:
                ...
                if hover_card:
                    ...
                    try:
                        following_els = hover_card.find_elements(
                            "xpath", './/a[contains(@href, "/following")]//span'
                        )
                        if following_els:
                            self.following_cnt = following_els[0].text or "0"
                    except Exception:
                        pass

                    try:
                        follower_els = hover_card.find_elements(
                            "xpath", './/a[contains(@href, "/followers")]//span'
                        )
                        if not follower_els:
                            follower_els = hover_card.find_elements(
                                "xpath", './/a[contains(@href, "/verified_followers")]//span'
                            )
                        if follower_els:
                            self.followers_cnt = follower_els[0].text or "0"
                    except Exception:
                        pass
